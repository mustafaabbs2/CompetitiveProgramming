...
// => S = S - node.value Make two recursive calls for both the children of the
// current node with the new number calculated in the previous step. At every
// step, see if the current node being visited is a leaf node and if its value
// is equal to the given number ‘S’. If both these conditions are true, we have
// found the required root-to-leaf path, therefore return true. If the current
// node is a leaf but its value is not equal to the given number ‘S’, return
// false.
