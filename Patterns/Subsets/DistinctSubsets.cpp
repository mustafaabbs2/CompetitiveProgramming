...
// [[], [1]]; Add the second number (5) to all the existing subsets: [[], [1],
// [5], [1,5]]; Add the third number (3) to all the existing subsets: [[], [1],
// [5], [1,5], [3], [1,3], [5,3], [1,5,3]].
...
public:
    static vector<vector<int>> findSubsets(const vector<int>& nums)
    {
        ...
        {
            size_t n = subsets.size(); // find the new size of the subset list
            // for each number, add combinations of all the other numbers
            for (size_t i = 0; i < n; i++) // add that number to all the subsets
            {
                vector<int> set(subsets[i]); // initialize set with the values
                                             // from subset[i]
                set.push_back(number); // every number will be added to a new
                                       // set
